...
program
  .name('genass')
  .description('AI-powered asset generation CLI with enhanced interactive UI')
  .version('1.0.0');
...
program
  ...
  .action(async (options) => {
    try {
      console.log(chalk.hex('#FF6B35').bold('🚀 GenAss') + chalk.gray(' - AI Asset Generator'));
...
program
  .command('chat')
  .alias('interactive')
  .description('Start interactive AI-powered session with enhanced Gemini CLI-style UI')
  .option('-p, --path <path>', 'Project path', process.cwd())
  .option('--classic', 'Use classic interactive mode instead of enhanced UI')
  .action(async (options) => {
    try {
      if (!validateEnvironment()) {
        process.exit(1);
      ...
      if (options.classic) {
        ...
        const session = new EnhancedInteractiveSession(options.path);
        await session.start();
      }
    } catch (error) {
...
// If no command is provided, start enhanced interactive mode by default
if (process.argv.length === 2) {
  (async () => {
    try {
      ...
      const session = new EnhancedInteractiveSession(process.cwd());
      await session.start();
    } catch (error) {
      logger.error('Interactive session failed:', error);
      console.error(chalk.red('❌ Failed to start interactive session'));
